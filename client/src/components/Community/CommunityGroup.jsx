  ...
  useEffect(() => {
    const fetchUserAndGroup = async () => {
      try {
        ...
        const groupResponse = await axios.get(`https://connectify-93bj.onrender.com/community/${communityId}/group`, { withCredentials: true });
...
const handleSendMessage = async () => {
  ...
  if (!user || !user._id) {
    console.warn("User not loaded yet, cannot send message.");
    return;
  }
  ...
  try {
    ...
    await axios.post(
      `https://connectify-93bj.onrender.com/community/${communityId}/group`,
  ...
  useEffect(() => {
    const fetchCommunityName = async () => {
      try {
        const response = await axios.post(`https://connectify-93bj.onrender.com/community/${communityId}`, { communityId } , { withCredentials: true });
  ...
  <div
    key={index}
    className={`flex mb-2 ${
      user && msg.senderId === user._id ? "justify-end" : "justify-start"
    }`}
  >
    <div
      className={`p-3 rounded-lg max-w-xs ${
        user && msg.senderId === user._id ? "bg-blue-800" : "bg-gray-700"
      }`}
    >
